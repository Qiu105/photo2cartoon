...
class Photo2Cartoon(object):
    def __init__(self, args):
        ...
        """ Weight """
        self.adv_weight = 1
        self.cycle_weight = 50
        self.faceid_weight = 1
    ...
    def build_model(self):
        ...
        self.genA2B = ResnetGenerator(ngf=self.channel, img_size=self.img_size).to(self.device)
        self.genB2A = ResnetGenerator(ngf=self.channel, img_size=self.img_size).to(self.device)
        self.disGA = Discriminator(input_nc=3, ndf=self.channel, n_layers=7).to(self.device)
        self.disGB = Discriminator(input_nc=3, ndf=self.channel, n_layers=7).to(self.device)
        self.disLA = Discriminator(input_nc=3, ndf=self.channel, n_layers=5).to(self.device)
        self.disLB = Discriminator(input_nc=3, ndf=self.channel, n_layers=5).to(self.device)
        self.facenet = FaceFeatures('models/model_mobilefacenet.pth', self.device)
        # summary(self.genA2B, (3, 256, 256))
        # summary(self.genB2A, (3, 256, 256))
        # summary(self.disGA, (3, 256, 256))
        # summary(self.disGB, (3, 256, 256))
        # summary(self.disLA, (3, 256, 256))
        # summary(self.disLB, (3, 256, 256))
            ...
            for step in range(start_iter, self.iteration + 1):
                ...
                if step % 10 == 0:
                    print("epoch %2d :[%5d/%5d] time: %4.4f d_loss: %.8f, g_loss: %.8f" % (
                        epoch, step, self.iteration, time.time() - start_time, Discriminator_loss, Generator_loss))
    ...
    def test(self):
        ...
        with torch.no_grad():
            for n, (real_A, _) in enumerate(self.testA_loader):
                ...
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'A2B_%d.png' % (n + 1)),
                            A2B * 255.0)
            ...
            for n, (real_B, _) in enumerate(self.testB_loader):
                ...
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B2A_%d.png' % (n + 1)),
                            B2A * 255.0)
